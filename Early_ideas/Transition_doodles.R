#### More transitions doodles, trying to get comparisons to differing states #####
...
identify.novel.gam <- function(site.sp.mat, alpha,
  ...
  # FIXED REFERENCE DISSIMILARITY
  # calculate differenced sequential dissimilarities between 
  # time t and time novel t-1
  
  pre_novelty <- (site.dist) %>% 
    dplyr::select(starts_with("novel"))
  
  fixed_point_dist <-unname(unlist(pre_novelty))
  
  fixed_point_dist[fixed_point_dist == 0] <- NA
  
  # transform to remove 0s and 1s for beta regression
  fixed_point_dist_tr <- (fixed_point_dist * (length(fixed_point_dist)-1) + 0.5) / length(fixed_point_dist)
  
  # model localised trend over time and extract residuals to get dissimilarity 
  # compared to local mean.
  if(var(fixed_point_dist_tr, na.rm=TRUE)==0){return(NULL)}
  
  if(!is.na(set.k)){
    fixed.gam <- gam(fixed_point_dist_tr ~ s(bins, bs="cr", k= set.k), 
                     family=betar(),
                     method="REML")
  } else{
    fixed.gam <- gam(fixed_point_dist_tr ~ bins, family=betar(), method="REML")
  }
  
  fix.mu <- c(NA, fixed.gam $fitted.values)
  phi <- as.numeric(substr(fixed.gam$family$family,
                           regexpr("\\(", fixed.gam$family$family)+1,
                           nchar(fixed.gam$family$family)-1))
  ...
  fix.p <- do.call("rbind", lapply(1:length(A), function(n){
    data.frame(lwr=qbeta(0.05, shape1=A[n], shape2=B[n]),
               upr=qbeta(0.95, shape1=A[n], shape2=B[n]),
               fix.p = pbeta(fixed_point_dist_tr[n], shape1=A[n], shape2=B[n], lower.tail=FALSE))
  }))
  ...
  if(plot){
    ...
    plot(fixed_point_dist ~ bins, type="n",
         ylim=c(max(fixed_point_dist, na.rm=TRUE)+0.1, 
                min(fixed_point_dist, na.rm=TRUE)),
         axes=FALSE)
    
    polygon(x=c(bins, rev(bins)),
            y=c(fix.p[,1], rev(fix.p[,2])), 
            col="grey80", border=NA)
    lines(plogis(predict(fixed.gam)) ~ bins[-1], col="grey50", lwd=2)
    lines(fixed_point_dist ~ bins)
