...
identify.novel.gam.MDS<-function(site.sp.mat, alpha, 
  ...
  if(plot){
    par(mfrow=c(2,2), mar=c(0,0,0,0), oma=c(3,6,0.5,3), las=1)
    ...
    legend(x=par("usr")[2], y= par("usr")[4],
           legend=c("Instant", "Cumulative", "Novel"),
    ...
    site.sp.mat$category <- return.data$cat
    site.sp.mat$colour <- NA
    
    for (i in 1:nrow(site.sp.mat)) {
      if(site.sp.mat$category[i] == "cumul"){
        site.sp.mat$colour[i] <- "skyblue"
      }
      if(site.sp.mat$category[i] == "instant"){
        site.sp.mat$colour[i] <- "red1"
      }
      if(site.sp.mat$category[i] == "novel"){
        site.sp.mat$colour[i] <- "orange"
      }
      if(site.sp.mat$category[i] == "back"){
        (site.sp.mat$colour[i] <- "grey")}
    }
    
    NMDS_2=metaMDS(site.sp.mat[,-c(ncol(site.sp.mat), (ncol(site.sp.mat)-1))], 
                   k=2, trymax = 10000)
    plot(NMDS_2, type = "n")
    points(x = NMDS_2$points[,"MDS1"], 
           y = NMDS_2$points[, "MDS2"], 
           col = site.sp.mat$colour,
           pch = 16,
           cex = 1.1)
    
    for (i in 1:(nrow(NMDS_2$points)-1)){
      
      arrows(x0 = NMDS_2$points[i,"MDS1"], 
             y0 = NMDS_2$points[i, "MDS2"], 
             x1 = NMDS_2$points[i+1,"MDS1"], 
             y1 = NMDS_2$points[i+1, "MDS2"], length = 0.05, lwd = 1)
    }
