local.extorig.RivFishTime <- function(ssmat,
                          novel,
                          site.name){
...
# new function which differentiates between native and non-natives!
invaders.extorig.RivFishTime <- function(ssmat,
                                      novel,
                                      site.name){
  
  ## Add NNC
  ssmat<-ssmat[stri_detect_fixed(colnames(ssmat), "nvader")]
  #ssmat.exo<-ssmat[stri_detect_fixed(colnames(ssmat), "xotic")]
 # ssmat<-cbind(ssmat.exo, ssmat.inv)
  # Check to see if age of site-species matrices increase
  # (first row is oldest)
  if(as.numeric(as.character(rownames(ssmat)[1])) < 
     as.numeric(as.character(rownames(ssmat)[dim(ssmat)[1]]))){
  ...
  count.df <- do.call("rbind", lapply(1:dim(bin.mat)[1], function(bin){
    ...
    # for extinctions, get everything from target to end of time-series
    ext.mat <- as.matrix(bin.mat[target["ext"]:dim(bin.mat)[1],])
    
    # for originations, get everything from 1st entry to target
    orig.mat <- as.matrix(bin.mat[1:target["orig"],])
