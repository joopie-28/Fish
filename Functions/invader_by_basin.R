...
test <- merge(full.novel.mat.season, InvByBasin, by=c("basin","bins"))

summary(glmer(novel ~ bin_lag + position + INC_increase*total_inv + (1|site_ID/Quarter), 
              data = test, 
              family = 'binomial')) # significant interaction


# figure out how that relates to geo.timeseries.full
