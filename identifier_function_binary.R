...
binary_converter_function <- function(survey_identifier){
  ...
  for (i in 1:nrow(binary_df)) {
    if (length(as.numeric(rownames(binary_df[i,]))) == 0){
      return("Unable to complete analysis as this character is logical class")
    }
  ...
  # Selecting only those that occur in the MOST CONSISTENT QUARTER then deleting the quarter column
...
novel_results_df <- identify.novel.gam(site.sp.mat = binary_converter_function(survey_identifier), alpha = 0.05, metric = "jaccard", site = survey_identifier, plot = TRUE, plot.data = FALSE,
                           gam.max.k = -1)

# Remove first five rows as per Pandolfi method, we just use a simple line for that here. Note that the plots still show these
# first 5 bins so any novel communities that show up early there need to be ignored.
novel_results_df <- novel_results_df[-c(1:5),]
...
A <- binary_converter_function(survey_identifier)
