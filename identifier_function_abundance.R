# Identifier function abundance 

# Exactly the same as the binary function only now we use the abundance data and a different index.
abundance_identifier_function <- function(survey_identifier){
  ...
  # Changing the name here because I'm recycling code I wrote earlier, a bit messy but saves time.
  ...
  #  Selecting only those that occur in quarter 3, then deleting the quarter column. This is an arbitrary
  # decision and will need to be reviewed
  binary_df <- subset(binary_df, quarter == 3)
...
survey_identifier <- "G1037"


a <- abundance_identifier_function(survey_identifier)

test <- identify.novel.gam(site.sp.mat = a, alpha = 0.05, metric = "bray", site = survey_identifier, plot = TRUE, plot.data = FALSE, gam.max.k = -1)
test
